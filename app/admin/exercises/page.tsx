  ...
  return (
    <div className="space-y-5">
      ...
      <AdminFormSection title="Informations générales" description="Donnez un nom clair et une description utile aux coachs." icon={<Info className="h-5 w-5" aria-hidden="true" />}>
        <div className="space-y-2"><Label htmlFor="exercise-name">Nom de l’exercice *</Label><Input id="exercise-name" value={draft.name} onChange={(event) => onChange({ name: event.target.value })} placeholder="Ex. Développé couché" className="h-11 bg-muted/30" aria-invalid={errors.some((error) => error.field === "exercise-name")} disabled={disabled} /></div>
        <div className="space-y-2"><Label htmlFor="exercise-description">Description <span className="font-normal text-muted-foreground">(optionnel)</span></Label><Textarea id="exercise-description" value={draft.description} onChange={(event) => onChange({ description: event.target.value })} placeholder="Décrivez l’exécution, le placement et les points de vigilance." className="min-h-24 resize-y bg-muted/30" disabled={disabled} /></div>
      </AdminFormSection>
      ...
      <AdminFormSection title="Classification" description="Le groupe musculaire est requis par le catalogue actuel." icon={<Dumbbell className="h-5 w-5" aria-hidden="true" />}>
        <div className="space-y-2"><Label>Groupe musculaire *</Label><Select value={draft.muscleGroup || undefined} onValueChange={(value) => onChange({ muscleGroup: value })} disabled={disabled}><SelectTrigger className="h-11 w-full bg-muted/30" aria-invalid={errors.some((error) => error.field === "exercise-muscle-group")}><SelectValue placeholder="Sélectionner le groupe musculaire" /></SelectTrigger><SelectContent>{muscleGroups.map((group) => <SelectItem key={group} value={group}>{group}</SelectItem>)}</SelectContent></Select></div>
        <p className="rounded-xl border border-border bg-muted/20 p-3 text-sm text-muted-foreground">Équipement, difficulté, type et sexe ne sont pas ajoutés ici car le contrat API actuel ne les expose pas dans ce workflow.</p>
      </AdminFormSection>
      ...
      <AdminFormSection title="Paramètres par défaut" description="Ces valeurs restent facultatives et pourront être ajustées dans chaque séance." icon={<SlidersHorizontal className="h-5 w-5" aria-hidden="true" />}>
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2"><Label htmlFor="exercise-sets">Séries</Label><Input id="exercise-sets" type="number" min={1} value={draft.sets} onChange={(event) => onChange({ sets: event.target.value })} placeholder="Optionnel" className="h-11 bg-muted/30" disabled={disabled} /></div>
          <div className="space-y-2"><Label htmlFor="exercise-reps">Répétitions</Label><Input id="exercise-reps" type="number" min={1} value={draft.reps} onChange={(event) => onChange({ reps: event.target.value })} placeholder="Optionnel" className="h-11 bg-muted/30" disabled={disabled} /></div>
          <div className="space-y-2"><Label htmlFor="exercise-rest">Repos</Label><div className="relative"><Input id="exercise-rest" type="number" min={0} value={draft.restTime} onChange={(event) => onChange({ restTime: event.target.value })} placeholder="Optionnel" className="h-11 bg-muted/30 pr-12" disabled={disabled} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">sec</span></div></div>
          <div className="space-y-2"><Label htmlFor="exercise-weight">Poids conseillé</Label><div className="relative"><Input id="exercise-weight" type="number" min={0} step={0.5} value={draft.defaultWeight} onChange={(event) => onChange({ defaultWeight: event.target.value })} placeholder="Optionnel" className="h-11 bg-muted/30 pr-12" disabled={disabled} /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">kg</span></div></div>
        </div>
      </AdminFormSection>
      ...
      {onVideoSelect && (
        <AdminFormSection title="Média" description="Ajoutez une vidéo de démonstration sans lecture automatique." icon={<ImageIcon className="h-5 w-5" aria-hidden="true" />}>
          ...
          <label htmlFor="exercise-video-upload" className="flex min-h-32 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border bg-muted/20 p-5 text-center transition-colors hover:border-primary hover:bg-primary/5 focus-within:ring-2 focus-within:ring-ring">
            <Upload className="h-6 w-6 text-muted-foreground" aria-hidden="true" /><span className="text-sm font-medium text-foreground">{selectedVideoFile?.name ?? "Choisir une vidéo"}</span><span className="text-xs text-muted-foreground">MP4, MOV, AVI ou WEBM · 100 Mo maximum</span>
          </label>
          ...
          {disabled && selectedVideoFile && <div className="space-y-2" aria-live="polite"><div className="flex justify-between text-sm text-muted-foreground"><span>Création et envoi…</span><span>{uploadProgress ?? 0}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary transition-[width]" style={{ width: `${uploadProgress ?? 0}%` }} /></div></div>}
        </AdminFormSection>
      )}
    </div>
  )
